% gppi_control_cortical_only.m
% Control ROI-to-ROI HRF-deconvolved gPPI analysis restricted to cortical ROIs.
% Circuit estimates are undirected averages of both ROI-to-ROI directions.
...
CORTICAL_ROI_DIR = request_existing_directory('Enter the cortical ROI-mask directory: ');
RESULT_ROOT = request_output_directory('Enter the output directory: ');
OUT_XLSX = fullfile(RESULT_ROOT, 'gppi_control_cortical_only_results.xlsx');
...
CORTICAL_SYSTEM_RULES = {
    'DLPFC',             {'^DLPFC_L$', '^DLPFC_R$'};
    'InferiorFrontal',   {'^InferiorFrontal_L$', '^InferiorFrontal_R$'};
    'MedialFrontal',     {'^MedialFrontal_L$', '^MedialFrontal_R$'};
    'FrontalControl',    {'^DLPFC_', '^InferiorFrontal_', '^MedialFrontal_'};
    ...
    'CorticalAll',       {'^DLPFC_', '^InferiorFrontal_', '^MedialFrontal_', '^MotorSensorimotor_', '^Precuneus_', '^VisualOccipital_'}
};

PRIMARY_CIRCUITS = {
    'FrontalControl_to_VisualOccipital',     'FrontalControl',    'VisualOccipital';
    'FrontalControl_to_Precuneus',           'FrontalControl',    'Precuneus';
    'FrontalControl_to_MotorSensorimotor',   'FrontalControl',    'MotorSensorimotor';
    'VisualOccipital_to_Precuneus',          'VisualOccipital',   'Precuneus';
    'VisualOccipital_to_MotorSensorimotor',  'VisualOccipital',   'MotorSensorimotor';
    'Precuneus_to_MotorSensorimotor',        'Precuneus',         'MotorSensorimotor'
};

SECONDARY_CIRCUITS = {
    'DLPFC_to_InferiorFrontal',              'DLPFC',             'InferiorFrontal';
    'DLPFC_to_MedialFrontal',                'DLPFC',             'MedialFrontal';
    'InferiorFrontal_to_MedialFrontal',      'InferiorFrontal',   'MedialFrontal';
    'DLPFC_to_VisualOccipital',              'DLPFC',             'VisualOccipital';
    'InferiorFrontal_to_VisualOccipital',    'InferiorFrontal',   'VisualOccipital';
    'MedialFrontal_to_VisualOccipital',      'MedialFrontal',     'VisualOccipital';
    'DLPFC_to_Precuneus',                    'DLPFC',             'Precuneus';
    'InferiorFrontal_to_Precuneus',          'InferiorFrontal',   'Precuneus';
    'MedialFrontal_to_Precuneus',            'MedialFrontal',     'Precuneus';
    'DLPFC_to_MotorSensorimotor',            'DLPFC',             'MotorSensorimotor';
    'InferiorFrontal_to_MotorSensorimotor',  'InferiorFrontal',   'MotorSensorimotor';
    'MedialFrontal_to_MotorSensorimotor',    'MedialFrontal',     'MotorSensorimotor';
    'FrontalControl_to_PosteriorRetrieval',  'FrontalControl',    'PosteriorRetrieval'
};

fprintf('\nLoading cortical ROIs as seeds and targets:\n%s\n', CORTICAL_ROI_DIR);
seedList = load_roi_list(CORTICAL_ROI_DIR);
targetList = seedList;
fprintf('Cortical ROIs found: %d\n', numel(seedList));

seedMembership = build_system_membership(seedList, CORTICAL_SYSTEM_RULES, 'seed');
targetMembership = build_system_membership(targetList, CORTICAL_SYSTEM_RULES, 'target');
...
notes = {
    'Analysis label', 'Cortical target-to-target undirected circuit-first HRF-deconvolved ROI-to-ROI gPPI';
    ...
    'Primary correction family', 'six primary cortical circuits x primary measures x active frequencies';
    'Circuit directionality', 'undirected circuit averages; edge follow-up keeps directed ROI-to-ROI estimates';
...
fprintf('\n=============================================\n');
fprintf('Cortical target-to-target gPPI finished.\n');
...
function circuitTable = build_circuit_beta_table(edgeTable, circuitDefTable, seedMembership, targetMembership, measures, minEdges)
    ...
    for s = 1:height(sessionList)
        ...
        for c = 1:height(circuitDefTable)
            ...
            nodeA = string(circuitDefTable.seed_system(c));
            nodeB = string(circuitDefTable.target_system(c));
            isPrimary = logical(circuitDefTable.is_primary(c));

            namesA_seed = system_members(seedMembership, nodeA);
            namesB_seed = system_members(seedMembership, nodeB);
            namesA_target = system_members(targetMembership, nodeA);
            namesB_target = system_members(targetMembership, nodeB);

            idxAB = string(edgeTable.subject) == subj & ...
                    string(edgeTable.stim) == stim & ...
                    string(edgeTable.target_group) == tg & ...
                    string(edgeTable.status) == "success" & ...
                    ismember(string(edgeTable.seed), namesA_seed) & ...
                    ismember(string(edgeTable.target_roi), namesB_target);

            idxBA = string(edgeTable.subject) == subj & ...
                    string(edgeTable.stim) == stim & ...
                    string(edgeTable.target_group) == tg & ...
                    string(edgeTable.status) == "success" & ...
                    ismember(string(edgeTable.seed), namesB_seed) & ...
                    ismember(string(edgeTable.target_roi), namesA_target);
            ...
            rows(end+1, :) = [{char(subj), char(stim), char(tg), char(circuit), char(nodeA), char(nodeB), isPrimary, nEdges, status, reason}, vals]; %#ok<AGROW>
        end
    end
...
function edgeStats = run_edge_followup_delta(edgeTable, circuitDefTable, seedMembership, targetMembership, measures, activeStims, offStim)
    ...
    rows = {};
    vars = {'family','measure','active_stim','circuit','seed','target_roi','direction_in_circuit','n_SN','n_STN','mean_delta_SN','mean_delta_STN','diff_SN_minus_STN','t','df','p','p_fdr_within_circuit_measure'};
    ...
    for c = 1:height(circuitDefTable)
        ...
        [pairSeeds, pairTargets, pairDirections] = undirected_edge_pairs(namesA_seed, namesB_target, namesB_seed, namesA_target, nodeA, nodeB);

        for m = 1:numel(measures)
            measure = measures{m};
            for e = 1:numel(pairSeeds)
                seed = pairSeeds(e);
                targetROI = pairTargets(e);
                directionLabel = pairDirections(e);
                ...
                for a = 1:numel(activeStims)
                    ...
                    rows(end+1, :) = {
                        'edge_followup_delta', measure, char(active), char(circuit), char(seed), char(targetROI), char(directionLabel), ...
...
function [pairSeeds, pairTargets, pairDirections] = undirected_edge_pairs(namesA_seed, namesB_target, namesB_seed, namesA_target, nodeA, nodeB)

    pairSeeds = strings(0, 1);
    pairTargets = strings(0, 1);
    pairDirections = strings(0, 1);

    for i = 1:numel(namesA_seed)
        for j = 1:numel(namesB_target)
            if namesA_seed(i) ~= namesB_target(j)
                pairSeeds(end+1, 1) = namesA_seed(i); %#ok<AGROW>
                pairTargets(end+1, 1) = namesB_target(j); %#ok<AGROW>
                pairDirections(end+1, 1) = nodeA + "_to_" + nodeB; %#ok<AGROW>
            end
        end
    end

    for i = 1:numel(namesB_seed)
        for j = 1:numel(namesA_target)
            if namesB_seed(i) ~= namesA_target(j)
                pairSeeds(end+1, 1) = namesB_seed(i); %#ok<AGROW>
                pairTargets(end+1, 1) = namesA_target(j); %#ok<AGROW>
                pairDirections(end+1, 1) = nodeB + "_to_" + nodeA; %#ok<AGROW>
            end
        end
    end

    if ~isempty(pairSeeds)
        T = table(pairSeeds, pairTargets, pairDirections);
        T = unique(T);
        pairSeeds = T.pairSeeds;
        pairTargets = T.pairTargets;
        pairDirections = T.pairDirections;
    end
end
...
function edgeStats = run_edge_followup_behavior(edgeTable, behTable, circuitDefTable, seedMembership, targetMembership, measures, activeStims, offStim, behaviorMetric)
    ...
    rows = {};
    vars = {'family','measure','active_stim','behavior_metric','circuit','seed','target_roi','direction_in_circuit','n','n_SN','n_STN', ...
        ...
        for m = 1:numel(measures)
            ...
            for e = 1:numel(pairSeeds)
                ...
                for a = 1:numel(activeStims)
                    ...
                    rows(end+1, :) = {
                        'edge_followup_behavior', measure, char(active), behaviorMetric, char(circuit), char(seed), char(targetROI), char(directionLabel), ...
                        numel(y), sum(g == 1), sum(g == 0), slopeSTN, slopeSN, betaInt, tInt, pInt, NaN
                    }; %#ok<AGROW>
                end
            end
