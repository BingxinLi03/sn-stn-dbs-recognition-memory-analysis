% gppi_control_deep_hippocampal_to_frontal.m
% Control ROI-to-ROI HRF-deconvolved gPPI analysis of deep or hippocampal seeds and frontal targets.
...
OUT_XLSX = fullfile(RESULT_ROOT, 'gppi_control_deep_hippocampal_to_frontal_results.xlsx');
...
TARGET_SYSTEM_RULES = {
    'FrontalControl',      {'^DLPFC(_|$)', '^InferiorFrontal(_|$)', '^MedialFrontal(_|$)'};
    'DLPFC',               {'^DLPFC(_|$)'};
    'InferiorFrontal',     {'^InferiorFrontal(_|$)'};
    'MedialFrontal',       {'^MedialFrontal(_|$)'}
};

PRIMARY_CIRCUITS = {
    'TargetMatched_to_FrontalControl',       'TargetMatched',      'FrontalControl';
...
SECONDARY_CIRCUITS = {
    ...
    'DeepNucleiAll_to_FrontalControl',       'DeepNucleiAll',      'FrontalControl';
    'ThalamicNuclei_to_FrontalControl',      'ThalamicNuclei',     'FrontalControl';

    'TargetMatched_to_DLPFC',                'TargetMatched',      'DLPFC';
    'TargetMatched_to_InferiorFrontal',      'TargetMatched',      'InferiorFrontal';
    'TargetMatched_to_MedialFrontal',        'TargetMatched',      'MedialFrontal';

    'BasalGanglia_to_DLPFC',                 'BasalGanglia',       'DLPFC';
    'BasalGanglia_to_InferiorFrontal',       'BasalGanglia',       'InferiorFrontal';
    'BasalGanglia_to_MedialFrontal',         'BasalGanglia',       'MedialFrontal';

    'HippocampalSystem_to_DLPFC',            'HippocampalSystem',  'DLPFC';
    'HippocampalSystem_to_InferiorFrontal',  'HippocampalSystem',  'InferiorFrontal';
    'HippocampalSystem_to_MedialFrontal',    'HippocampalSystem',  'MedialFrontal'
};
...
sourceSeedSystems = {'SN','STN','DBSTargetNuclei','BasalGanglia','ThalamicNuclei','HippocampalSystem','DeepNucleiAll'};
sourceTargetSystems = {'FrontalControl','DLPFC','InferiorFrontal','MedialFrontal'};
...
if isempty(seedNamesToKeep)
    error('No seed ROI matched deep nuclei / hippocampal-system rules. Check SEED_SYSTEM_RULES and ROI names.');
end
if isempty(targetNamesToKeep)
    error('No target ROI matched frontal rules. Check TARGET_SYSTEM_RULES and ROI names.');
end
...
fprintf('Seeds kept for frontal analysis: %d\n', numel(seedList));
fprintf('Frontal targets kept for frontal analysis: %d\n', numel(targetList));
...
primaryOffBehaviorStats = run_primary_off_behavior_interaction_stats( ...
    circuitTable, behaviorTable, PRIMARY_MEASURES, ACTIVE_STIMS, STIM_OFF, PRIMARY_BEHAVIOR);
primaryOffBehaviorStats = add_fdr_one_family(primaryOffBehaviorStats, 'p_interaction', 'p_fdr_primary');

if RUN_PERMUTATION && height(primaryOffBehaviorStats) > 0
    primaryOffBehaviorStats = add_permutation_maxstat_off_behavior_interaction( ...
        primaryOffBehaviorStats, circuitTable, behaviorTable, PRIMARY_MEASURES, ACTIVE_STIMS, STIM_OFF, PRIMARY_BEHAVIOR, N_PERM);
end
...
writetable(primaryOffBehaviorStats, OUT_XLSX, 'Sheet', 'primary_OFF_predicts_FAR');
...
notes = {
    'Analysis label', 'Deep nuclei / hippocampal-system to frontal-control circuit-first HRF-deconvolved ROI-to-ROI gPPI';
...
fprintf('\n=============================================\n');
fprintf('Frontal deep/hippocampal gPPI finished.\n');
...
function circuitTable = build_circuit_beta_table(edgeTable, circuitDefTable, seedMembership, targetMembership, measures, minEdges)
    ...
    for s = 1:height(sessionList)
        ...
        for c = 1:height(circuitDefTable)
            ...
            seedNames = seed_members_for_circuit(seedMembership, seedSystem, tg);
...
function names = seed_members_for_circuit(seedMembership, seedSystem, targetGroup)

    seedSystem = string(seedSystem);
    targetGroup = string(targetGroup);

    if seedSystem == "TargetMatched"
        if targetGroup == "SN"
            names = system_members(seedMembership, "SN");
        elseif targetGroup == "STN"
            names = system_members(seedMembership, "STN");
        else
            names = strings(0, 1);
        end
    else
        names = system_members(seedMembership, seedSystem);
    end
end
...
function statsTable = run_primary_off_behavior_interaction_stats(circuitTable, behTable, measures, activeStims, offStim, behaviorMetric)
    ...
    for m = 1:numel(measures)
        ...
        for c = 1:numel(primaryCircuits)
            ...
            for a = 1:numel(activeStims)
                ...
                [x, y, g] = paired_circuit_off_behavior_delta(circuitTable, behTable, measure, behaviorMetric, circuit, active, string(offStim));
                [lm, ok] = fit_interaction_lm(x, y, g);
                [rSN, pSN] = corr_by_group(x, y, g, 1);
                [rSTN, pSTN] = corr_by_group(x, y, g, 0);
                looSN = loo_min_abs_corr(x(g == 1), y(g == 1));
                ...
                rows(end+1, :) = {
                    'primary_OFF_behavior_interaction', measure, char(active), behaviorMetric, char(circuit), ...
                    numel(y), sum(g == 1), sum(g == 0), slopeSTN, slopeSN, betaInt, tInt, pInt, ...
                    rSN, pSN, rSTN, pSTN, looSN, looSTN, NaN, NaN
                }; %#ok<AGROW>
            end
...
function [x, y, g] = paired_circuit_off_behavior_delta(circuitTable, behTable, measure, behaviorMetric, circuit, activeStim, offStim)
    ...
    for i = 1:numel(subjects)
        ...
        rowOff = rows(string(rows.subject) == sub & string(rows.stim) == offStim, :);
        rowActive = rows(string(rows.subject) == sub & string(rows.stim) == activeStim, :);

        behA = behTable(string(behTable.subject) == sub & string(behTable.stim) == activeStim, :);
        behB = behTable(string(behTable.subject) == sub & string(behTable.stim) == offStim, :);

        if ~isempty(rowOff) && ~isempty(rowActive) && ~isempty(behA) && ~isempty(behB)
            xv = double(rowOff.(measure)(1));
            yv = compute_behavior_delta(behA(1, :), behB(1, :), behaviorMetric);
            tg = string(rowOff.target_group(1));

            if isfinite(xv) && isfinite(yv) && (tg == "SN" || tg == "STN")
                x(end+1, 1) = xv; %#ok<AGROW>
                y(end+1, 1) = yv; %#ok<AGROW>
                g(end+1, 1) = double(tg == "SN"); %#ok<AGROW>
            end
        end
    end
end

function T = add_permutation_maxstat_off_behavior_interaction(T, circuitTable, behTable, measures, activeStims, offStim, behaviorMetric, nPerm)
    ...
    fprintf('Permutation max-stat for primary OFF-baseline behavior interaction: %d permutations\n', nPerm);

    data = cell(height(T), 1);
    for r = 1:height(T)
        [x, y, g] = paired_circuit_off_behavior_delta(circuitTable, behTable, char(T.measure(r)), behaviorMetric, string(T.circuit(r)), string(T.active_stim(r)), string(offStim));
